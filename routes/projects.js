...
router.get("/", async (req, res) => {
  try {
    ...
    let query = `
      ...
      LEFT JOIN (
        SELECT project_id, JSONB_AGG(JSONB_BUILD_OBJECT('file_path', file_path, 'original_name', original_name)) as files
        FROM project_files
        GROUP BY project_id
  ...
  try {
    ...
    const projectResult = await pool.query(`
      SELECT
        ...
        u.username AS author_name,
        COALESCE(
          (SELECT JSONB_AGG(JSONB_BUILD_OBJECT('file_path', file_path, 'original_name', original_name))
           FROM project_files
